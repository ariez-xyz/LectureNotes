...
student = (name = "TheBruh69", kerberos_id = "bruh")
...
# ╔═╡ de5550c8-34c3-11eb-2306-0f8d5b9b71d6
...
html"""<iframe width="100%" height="450px" src="https://www.youtube.com/embed/gTGJ80HayK0?rel=0" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>"""
...
# ╔═╡ 8748cf84-34c4-11eb-071a-a3713d84445b
begin
	res = img
	for i in 0:100
		global res
		res = remove_in_each_row(res, 1:size(res, 1))
	end
	res
end
...
function remove_in_each_row_views(img, column_numbers)
	...
	for (i, j) in enumerate(column_numbers)
		# EDIT THE FOLLOWING LINE and split it into two lines
		# to avoid using `vcat`.
		first = @view img[i, 1:j-1]
		second = @view img[i, j+1:end]
		img′[i, 1:j-1] .= first
		img′[i, j:end] .= second
	end
	img′
...
Again 2 allocs per row were saved because when you assign a slice to something, a copy is created.
"""
...
begin
	...
	brightness(c::Gray) = mean((c.val))
	brightness(c::RGBA) = mean((c.r, c.g, c.b))
	brightness(c::RGBX) = mean((c.r, c.g, c.b))
end
...
function greedy_seam(energies, starting_pixel::Int)
	row = 2
	indices = ones(Int, size(energies, 1))
	indices[1] = starting_pixel
	while row <= size(energies, 1)
		options = energies[row, max(1, indices[row-1]-1):min(indices[row-1]+1,end)]
		optionsInd = max(1, indices[row-1]-1)
		ind = 1
		mn = 1
...
function least_energy(energies, i, j)
	...
	indices = max(1,j-1):min(size(energies,2),j+1)
	
	minimum = 9999
	index = 0
	
	for k in indices
		e, _ = least_energy(energies, i+1, k)
		if e < minimum
			index = k
			minimum = e
		end
	...
	return minimum + energies[i,j], index
...
function recursive_seam(energies, starting_pixel)
	...
	for i in 2:m
		_, seam[i] = least_energy(energies, i, seam[i-1])
	end
	seam
end
...
function memoized_least_energy(energies, i, j, memory)
	...
	if !haskey(memory, (i,j))
		# calculate value if not memoized
		if i == size(energies, 1)
			...
			for k in indices
				e = memoized_least_energy(energies, i+1, k, memory)
				if e < minimum
					index = k
					minimum = e
				end
...
function recursive_memoized_seam(energies, starting_pixel)
	...
	seam[1] = starting_pixel
	for i in 2:m
		indices = max(1,seam[i-1]-1):min(size(energies,2),seam[i-1]+1)
		minimum = 9999
		index = 0
		for j in indices
			if memory[i,j] < minimum
				index = j
				minimum = memory[i,j]
			end
		end
...
# ╔═╡ f053526e-3579-11eb-2922-f1ced024efbc


# ╔═╡ 4e3bcf88-f3c5-11ea-3ada-2ff9213647b7
md"Compute shrunk image: $(@bind shrink_dict CheckBox())"

# ╔═╡ 46db19bc-3577-11eb-13b1-a5beb1da0450
mem = Dict{Tuple{Int,Int}, Float64}()

# ╔═╡ 261dcec2-3577-11eb-2983-8f408e326e99
memoized_least_energy(brightness.(img), 1, 100, mem)

# ╔═╡ efad02bc-3577-11eb-3438-93c154bd9f5c
length(mem)

# ╔═╡ df1a6f9a-357a-11eb-2d26-177cf4463f47
recursive_memoized_seam(brightness.(img), 400)
...
function matrix_least_energy(energies, i, j, memory, indices)
	m, n = size(energies)
	
	if memory[i,j] == 69420.0
		# calculate value if not memoized
		if i == size(energies, 1)
			memory[i,j] = energies[i,j]
			indices[i,j] = j
		else
			inds = max(1,j-1):min(size(energies,2),j+1)

			minimum = 9999
			index = 0

			for k in inds
				e = matrix_least_energy(energies, i+1, k, memory, indices)
				if e < minimum
					index = k
					minimum = e
				end
...
# ╔═╡ faaa7b30-357d-11eb-29a1-57d25d9b7f59
en = brightness.(img)

# ╔═╡ bd9efcfa-357f-11eb-2f34-5b01d30603dd
memory = ones(size(en)) * 69420

# ╔═╡ dfb474e6-357f-11eb-0cd2-27b704366615
indices = zeros(Int, size(en))

# ╔═╡ 174a3c62-357e-11eb-0a5d-954b842286de
matrix_least_energy(en, 1, 1, memory, indices)

# ╔═╡ d26893ea-357e-11eb-19c2-11f262a25cf6
indices

# ╔═╡ 332683c6-3580-11eb-1f11-15fc30c21841


# ╔═╡ be7d40e2-f320-11ea-1b56-dff2a0a16e8d
function matrix_memoized_seam(energies, starting_pixel)
	memory = ones(size(energies)) * 69420
	indices = zeros(Int, size(energies)) # location => least energy.
	                                         # pass this every time you call memoized_least_energy.
	m, n = size(energies)
	
	matrix_least_energy(energies, 1, starting_pixel, memory, indices)
	
	seam = ones(Int, m)
	seam[1] = starting_pixel
	for i in 2:m
		seam[i] = indices[i, seam[i-1]]
	end
	seam
end

# ╔═╡ 9fb77060-357e-11eb-20c8-8133c215e567
matrix_memoized_seam(en, 1)
...
function least_energy_matrix(energies)
	...
	inds = copy(energies)
	m, n = size(A)
	inds[m,:] = [i for i in 1:n]
	for i in m-1 : -1 : 1
		for j in 1:n
			beginIndex = max(j-1,1)
			minimum, index = findmin(A[i+1, max(j-1,1):min(end,j+1)])
			A[i,j] += minimum
			inds[i,j] = beginIndex + index - 1
		end
	end
	return A, inds
end

# ╔═╡ 37c373b2-35a8-11eb-0bfb-75e32d364fc2
ens, dirs = least_energy_matrix(en)

# ╔═╡ df0e859e-35a8-11eb-3b71-139b729f51a5
ens

# ╔═╡ e0c938ac-35a8-11eb-22b3-5ff6d48fe7da
dirs

# ╔═╡ 484a57aa-35a8-11eb-244f-6912bcf937a0
en
...
function seam_from_precomputed_least_energy(energies, starting_pixel::Int)
	least_energies, dirs = least_energy_matrix(energies)
	m, n = size(least_energies)
	
	seam = ones(Int, m)
	seam[1] = starting_pixel
	for i = 2:m
		seam[i] = dirs[i-1, seam[i-1]]
	end
	seam
end
...
# ╔═╡ e3071a3c-356f-11eb-2bdf-0dc1abb7d75f
visualize_seam_algorithm(recursive_seam, greedy_test, greedy_starting_pixel)

# ╔═╡ ddb75178-3574-11eb-0c12-635212efc85e
visualize_seam_algorithm(recursive_memoized_seam, greedy_test, greedy_starting_pixel)
