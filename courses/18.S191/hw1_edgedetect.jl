...
student = (name = "was ist den", kerberos_id = "jazz")
...
# ╔═╡ 4fe1c88a-2f35-11eb-285f-5dcfbc17873f
?colored_line
...
# ╔═╡ 77d9b51e-2f35-11eb-26b2-a915da5baad0
?rand
...
# ╔═╡ 9ed7ad8e-2f36-11eb-2d7f-d3ce5d31d238
fuk = copy(random_vect)

# ╔═╡ ab104c78-2f36-11eb-3d59-efa051c7486a
mean(fuk)

# ╔═╡ 12d010f2-2f37-11eb-31aa-6783d9063802
sum(fuk)

# ╔═╡ afb06e02-2f36-11eb-0712-9b8bb98720e9
demean(fuk)

# ╔═╡ 0925c86a-2f37-11eb-30c9-4b8dba921ecf
sum(fuk)
...
# ╔═╡ 40c16e1e-2f37-11eb-039f-e1f51520d572
create_bar()
...
# ╔═╡ 3fe44834-2f39-11eb-3a10-41015d36b045
x = vecvec_to_matrix([[1,2], [3,4]])

# ╔═╡ 45600062-2f39-11eb-0162-591d0229b1f9
x[1:2] .= [2,3]

# ╔═╡ 19440f30-2f3b-11eb-3f1a-234be746e5c1
x
...
function matrix_to_vecvec(matrix)
	m,n = size(matrix)
	vecvec = [zeros(n) for i in 1:m]
	for i in 1:m
		for j in 1:n
			vecvec[i][j] = matrix[i,j]
		end
	end
	return vecvec
end

# ╔═╡ d5c1b06e-2f3c-11eb-274c-d1b751602d01
...
function mean_colors(image)
	r = mean(map((x) -> x.r, image))
	g = mean(map((x) -> x.g, image))
	b = mean(map((x) -> x.b, image))
	return (r, g, b)
end
...
# ╔═╡ 42f25202-2f4b-11eb-2468-9d0a87ffab5d
10k

# ╔═╡ 44e8f994-2f4b-11eb-30c3-1b90f020e53d
...
begin
	...
	function invert(c::AbstractMatrix)
		return map(x -> invert(x), c)
	end
end
...
begin
	...
	function noisify(image::AbstractMatrix, s)
		...
		return map((x)->noisify(x, s), image)
	end
end

# ╔═╡ 11d198f8-2fd3-11eb-1994-a34a0d83a4a6
noisify(1,0.2)

# ╔═╡ 1703c6fe-2fd3-11eb-1c55-51b06290a8e0
noisify(RGB(0.1,1.0,0.1),0.1)
...
Need gt 1 because we add random noise in the interval (-s,s) so with s=1 the chance that a given pixel becomes a totally different color is actually not that high.
"""
...
# ╔═╡ c9663fc8-2f4c-11eb-3c41-6114e5c1d4d5
quantize(philip)
...
# ╔═╡ 52cfc4b4-2fd6-11eb-2d15-9f846ab613bd
extend(v,7)
...
function blur_1D(v, l)
	new_vec = [0.0 for i in 1:length(v)]
	for i in 1:length(v)
		sum = 0
		for j in -l:l
			sum += extend(v,i+j)
...
function convolve_vector(v, k)
	res = [0.0 for i in 1:length(v)]
	l = (length(k) - 1) ÷ 2
	for i in 1:length(v)
		sum = 0
		for j in -l:l
			sum += extend(v, i+j) * k[j+l + 1]
		end
		res[i] = sum
	end
	return res
...
begin
	...
	function gaussian_kernel(n)
		k = [0.0 for i in -n:n]
		for i in -n:n
			k[i + n + 1] = gaussian(i * 1.0)
		end
		return map((x) -> x / sum(k), k)
	end
end

# ╔═╡ 61af941a-3000-11eb-3a8c-6b93eecafd76
gaussian_kernel(1)

# ╔═╡ 71830284-3000-11eb-2424-61b2b05aacd0
...
begin
	function convolve_element(M, K, row, col)
		m, n = size(K)
		li = (m-1) ÷ 2
		lj = (n-1) ÷ 2
		
		el = extend_mat(M, -li + row, -lj + col) * K[1, 1]
		...
		for i in -li:li
			for j in -lj:lj
				...
				kj = j + lj + 1
				if i != -li || j != -lj
					el += extend_mat(M, i + row, j + col) * K[ki, kj]
				end
			end
		end
	...
	function convolve_image(M::AbstractMatrix, K::AbstractMatrix)
		...
		for i in 1:m
			for j in 1:n
				res[i,j] = convolve_element(M, K, i, j)
			end
		end
...
# ╔═╡ cb363f1c-3008-11eb-2f7f-7b33015d1916
test_image_with_border[6,6]
...
# ╔═╡ 9099a812-3008-11eb-144f-85bd2b67964c
convolve_element(test_image_with_border, K_test, 6,6)
...
begin
	...
	function gaussian_kernel_2d(n)
		...
		for i in -ln:ln
			for j in -ln:ln
				kernel[i + ln + 1,j + ln + 1] = gaussian(i,j)
			end
		end
		return kernel
	end
	...
	function with_gaussian_blur(image)
		return convolve_image(image, gaussian_kernel_2d(10))
	end
end

# ╔═╡ e51002f8-300b-11eb-2c8e-5515843bedce
...
# ╔═╡ c0ce37f6-3016-11eb-00ae-0bc5e875967f
function sobel(t)
	A = [0.125 0.0 -0.125
		 0.25 0 -0.25
	     0.125 0 -0.125]
...
# ╔═╡ 233b2e54-301b-11eb-0751-c99becf520d8
function norm(A)
	a = reduce((a,b)-> min(a,b), A)
	b = reduce((a,b)-> max(a,b), A)
	return map((x) -> (x - a)/(b - a), A)
end

# ╔═╡ de87e5a6-302c-11eb-19de-45d461d4c1b8
function channelsum(image)
	return norm(map((c) -> convert(AbstractFloat, c.r) + convert(AbstractFloat, c.b) + convert(AbstractFloat, c.g), image))
end

# ╔═╡ 9eeb876c-ee15-11ea-1794-d3ea79f47b75
function with_sobel_edge_detect(image)
	channel = channelsum(image)
	G1 = map((x)->x^2, convolve_image(channel, sobel(false)))
	G2 = map((x)->x^2, convolve_image(channel, sobel(true)))
	return norm(sqrt.(G1 + G2))
end

# ╔═╡ 1a8e2af0-301a-11eb-2735-730810637dda
Gray.(with_sobel_edge_detect(philip))

# ╔═╡ fa10d4fe-34c3-11eb-00cb-cb654304d298
Gray.(with_sobel_edge_detect(load(download("https://i.imgur.com/4SRnmkj.png"))))

# ╔═╡ d0dcdc6c-3037-11eb-0caa-3b3dacfdde68
md"""
# Reference image:
"""

# ╔═╡ 7365b1ae-3036-11eb-046e-e5d66ce83d64
...
# ╔═╡ 658b9e18-3036-11eb-03d9-112fe4f2720f
function convolve2(M, kernel, M_index_func=clamp_at_boundary)
...
# ╔═╡ 7cc57ad6-3036-11eb-1095-a3941cfd0b24
begin
	c1 = convolve2(channelsum(philip), Kernel.sobel()[1])
	c2 = convolve2(channelsum(philip), Kernel.sobel()[2])
	Gray.(norm(sqrt.(map((x)->x^2,c1) + map((x)->x^2,c2))))
end

# ╔═╡ 08c6994c-3097-11eb-299d-632ab80014f4
deli =load(download("https://cdn.shortpixel.ai/spai/w_1086+q_lossy+ret_img+to_webp/https://wisetoast.com/wp-content/uploads/2015/10/The-Persistence-of-Memory-salvador-deli-painting.jpg"))

# ╔═╡ 13b94b92-3097-11eb-048c-89af6f5eee90
Gray.(with_sobel_edge_detect(deli))

# ╔═╡ 4c8e3f8a-33f9-11eb-1707-c33e7bd895f6
md"""
# Seam carving
...
# ╔═╡ 79060cb8-33f5-11eb-1981-fd10cf92953b
